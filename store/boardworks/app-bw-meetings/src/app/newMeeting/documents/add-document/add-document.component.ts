...
export class AddDocumentComponent implements OnInit {
  @Input() addDocsaveClick: any;
  @Input() collaborationId: any;
  ...
  showLoader = false;
  @Output() onAddFileSaveSucess = new EventEmitter();
  ...
  ngOnChanges(): void {
    if (this.addDocsaveClick) {
      this.UplaodFileClick()
    }
  ...
  getCollaborationById() {
    if(this.collaborationId) {
      this.collaborationService.getCollaborationById(this.collaborationId.collaborationId).subscribe((data: any) => {
  ...
  UplaodFileClick() {
    ...
    if (this.fileListData.length === 0) {
      ...
      this.collaborationService.saveDocuments(this.collaborationId.collaborationId, reqObj).subscribe((data: any) => {
        //this.router.navigate(['admin/collaboration'])
        this.onAddFileSaveSucess.emit(true);
        this.addDocumentDataForm.reset();
