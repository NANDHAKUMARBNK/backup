  ...
  canActivate(
    ...
    state: RouterStateSnapshot): Observable<boolean | UrlTree> | Promise<boolean | UrlTree> | boolean | UrlTree {
      ...
      if (pagePermission.Collaboration) {
        let check = pagePermission.Collaboration.filter((element: any) => element.action == routePermision.action && (element.permission == routePermision.permission || element.permission == routePermision.allowPrivate ));
