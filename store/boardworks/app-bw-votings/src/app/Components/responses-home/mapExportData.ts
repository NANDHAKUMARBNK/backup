export class EvaluationExportData {
    static mapsummaryItems(item: any) {
        let csvRow: any = {};
        csvRow.title = item.title;
        csvRow.type = item.type;
        csvRow.startDate = item.startDate;
        csvRow.committeeId = item.committeeId;
        csvRow.status = item.status;
        // csvRow.action = item.action;
        return csvRow;
    }

    static mapItemsResponsesReport(item: any) {
